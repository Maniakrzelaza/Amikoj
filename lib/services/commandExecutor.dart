...
import 'package:amikoj/components/score_table.dart';
...
void execute(dynamic command, BuildContext ctx) async {
  ...
  switch (cmd) {
    case REDIRECT:
      ...
      if (meta != null) {
        RoomAction action =
            RoomAction.values.firstWhere((e) => e.toString() == meta['type']);
        args = {'type': action, 'roomName': meta['roomName']};
      }
      await Navigator.pushNamed(ctx, value, arguments: args);
      break;
